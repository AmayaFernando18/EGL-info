    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-background to-muted/20 py-8 px-4">
        <div className="max-w-7xl mx-auto">
          {/* Header Section */}
          <div className="mb-12">
            <div className="bg-gradient-to-br from-primary/10 via-white to-accent/10 rounded-3xl p-10 shadow-soft border border-primary/20">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                <div className="max-w-2xl">
                  <p className="uppercase text-sm font-semibold tracking-[0.3em] text-primary mb-3">Inside EGL</p>
                  <h1 className="text-4xl md:text-5xl font-black text-secondary leading-tight">
                    Visual stories from our energy journey
                  </h1>
                  <p className="text-lg text-gray-600 mt-4">
                    Explore milestone projects, field operations, and the teams powering Sri Lanka. Every frame is sourced from the EGL media archive to keep the intranet brand-authentic.
                  </p>
                </div>
                <div className="w-full max-w-sm bg-white rounded-2xl border border-border shadow-lg p-6">
                  <div className="grid grid-cols-2 gap-4 text-center">
                    <div>
                      <p className="text-xs uppercase text-gray-500 tracking-wide">Collections</p>
                      <p className="text-3xl font-bold text-secondary">12</p>
                      <span className="text-xs text-primary font-semibold">Curated sets</span>
                    </div>
                    <div>
                      <p className="text-xs uppercase text-gray-500 tracking-wide">Coverage</p>
                      <p className="text-3xl font-bold text-secondary">6</p>
                      <span className="text-xs text-accent font-semibold">Energy domains</span>
                    </div>
                  </div>
                  <div className="mt-6 flex flex-wrap gap-2 justify-center">
                    {['Solar', 'Wind', 'Hydro', 'Grid Ops', 'People'].map((label) => (
                      <span key={label} className="px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-semibold">
                        {label}
                      </span>
                    ))}
                  </div>

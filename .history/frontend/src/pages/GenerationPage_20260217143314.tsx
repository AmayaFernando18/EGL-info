...
export default function GenerationPage() {
  ...
  const generationDetails = [
    { label: 'CEB Hydro', value: 10.82, color: 'bg-primary', hex: '#0f9d58' },
    { label: 'CEB Thermal Coal', value: 16.83, color: 'bg-[#8c5a3c]', hex: '#8c5a3c' },
    { label: 'CEB Thermal Oil', value: 6.01, color: 'bg-[#c47324]', hex: '#c47324' },
    { label: 'CEB Wind', value: 0.86, color: 'bg-[#31b1c3]', hex: '#31b1c3' },
    { label: 'SPP Solar', value: 9.14, color: 'bg-[#f2c94c]', hex: '#f2c94c' },
    { label: 'SPP Biomass', value: 0.27, color: 'bg-[#2f855a]', hex: '#2f855a' },
    { label: 'SPP Minihydro', value: 1.6, color: 'bg-[#4caf50]', hex: '#4caf50' },
    { label: 'SPP Wind', value: 0.73, color: 'bg-[#7dd3fc]', hex: '#7dd3fc' },
    { label: 'IPP Thermal Oil', value: 4.45, color: 'bg-[#ef6c5b]', hex: '#ef6c5b' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section className="mb-10">
          <div className="flex flex-col space-y-6">
            <Card className="bg-white/90 backdrop-blur-sm border border-border shadow-xl">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-primary font-semibold">Official Dispatch</p>
                  <h2 className="text-3xl font-bold text-secondary mt-2">Daily Net Electricity Generation</h2>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center justify-center px-5 py-2 rounded-full bg-gradient-to-r from-primary to-accent text-white font-semibold shadow-soft hover:shadow-glow transition"
                >
                  More Info
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="rounded-2xl border border-border bg-gradient-to-br from-primary/5 to-white p-5 shadow-inner">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm font-semibold text-secondary uppercase tracking-wide">Total Net Energy</span>
                    <span className="text-3xl font-black text-secondary">{dailySummary.totalNetEnergy}</span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-xl bg-white/90 border border-primary/20 text-secondary p-4 shadow-sm">
                      <p className="text-xs uppercase text-primary font-semibold">Renewable</p>
                      <p className="text-2xl font-bold">{dailySummary.renewable.value}</p>
                      <p className="text-sm text-gray-500">{dailySummary.renewable.share}</p>
                    </div>
                    <div className="rounded-xl bg-[#1f2937] text-white p-4 shadow-sm">
                      <p className="text-xs uppercase text-white/70">Fossil Fuel</p>
                      <p className="text-2xl font-bold">{dailySummary.fossil.value}</p>
                      <p className="text-sm text-white/70">{dailySummary.fossil.share}</p>
                    </div>
                  </div>
                </div>

                <div className="rounded-2xl border border-border bg-gradient-to-br from-secondary/5 to-white p-5 shadow-inner">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm font-semibold text-secondary uppercase tracking-wide">Peak Demand</span>
                    <span className="text-3xl font-black text-secondary">{dailySummary.peakDemand}</span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-xl bg-white/90 border border-green-200 text-secondary p-4 shadow-sm">
                      <p className="text-xs uppercase text-green-700 font-semibold">Renewable</p>
                      <p className="text-2xl font-bold">{dailySummary.peakRenewable.value}</p>
                      <p className="text-sm text-gray-500">{dailySummary.peakRenewable.share}</p>
                    </div>
                    <div className="rounded-xl bg-gray-900 text-white p-4 shadow-sm">
                      <p className="text-xs uppercase text-white/70">Fossil Fuel</p>
                      <p className="text-2xl font-bold">{dailySummary.peakFossil.value}</p>
                      <p className="text-sm text-white/70">{dailySummary.peakFossil.share}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
                <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="flex flex-col items-center">
                    <div className="relative w-64 h-64 rounded-full shadow-lg border-8 border-white" style={pieGradientStyle}>
                      {highlightSource && (
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="bg-white/95 text-center px-4 py-3 rounded-xl shadow-lg border border-border">
                            <p className="text-xs uppercase text-muted-foreground tracking-wide">{highlightSource.label}</p>
                  ...
                  <div className="space-y-3">
                    {generationDetails.map((item) => (
                      <div key={item.label} className="flex items-center justify-between border border-border/80 rounded-xl px-4 py-2 bg-white shadow-sm">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="overflow-hidden rounded-2xl border border-border bg-white">
                      <table className="w-full text-sm text-secondary">
                        <tbody>
                          {energyLedger.map((row) => (
                            <tr key={row.label} className="border-b border-border last:border-0">
                              <td className="px-4 py-3 font-semibold text-gray-600">{row.label}</td>
                              <td className="px-4 py-3 text-right font-bold text-secondary">{row.value}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="rounded-2xl border border-border bg-gradient-to-br from-primary/5 to-white p-5 shadow-sm text-sm text-gray-700">
                    <p className="font-semibold text-secondary uppercase text-xs tracking-wide mb-3">Notes</p>
                    <ol className="list-decimal pl-5 space-y-2 mb-4 text-gray-600">
